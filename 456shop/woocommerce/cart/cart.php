				...
				if ( $_product->exists() && $values['quantity'] > 0 ) {
					...
					<tr class = "<?php echo esc_attr( apply_filters('woocommerce_cart_table_item_class', 'cart_table_item', $values, $cart_item_key ) ); ?>">
						...
						<td class="product-thumbnail">
							<?php
								$thumbnail = apply_filters( 'woocommerce_in_cart_product_thumbnail', $_product->get_image('shop-navi'), $values, $cart_item_key );
			...
			<td colspan="6" class="actions">
				...
				<?php if ( get_option( 'woocommerce_enable_coupons' ) == 'yes' && get_option( 'woocommerce_enable_coupon_form_on_cart' ) == 'yes') { ?>
					<div class="coupon">
						...
						<label for="coupon_code"><?php _e('Coupon', GETTEXT_DOMAIN); ?>:</label> <input type="text" name="coupon_code" id="coupon_code" value="" /> <input type="submit" class="button" name="apply_coupon" value="<?php _e('Apply Coupon', GETTEXT_DOMAIN); ?>" />
				...
				<input type="submit" class="button" name="update_cart" value="<?php _e('Update Cart', GETTEXT_DOMAIN); ?>" /> <input type="submit" class="checkout-button button1 btn btn-normal btn-primary" name="proceed" value="<?php _e('Proceed to Checkout &rarr;', GETTEXT_DOMAIN); ?>" />
