	...
	<table class="shop_table my_account_orders">
		...
		<thead>
			<tr>
				...
				<th class="order-shipto"><span class="nobr"><?php _e('Ship to', GETTEXT_DOMAIN); ?></span></th>
				<th class="order-total"><span class="nobr"><?php _e('Total', GETTEXT_DOMAIN); ?></span></th>
				<th class="order-status" colspan="2"><span class="nobr"><?php _e('Status', GETTEXT_DOMAIN); ?></span></th>
			</tr>
		</thead>
				...
				?><tr class="order">
					...
					<td class="order-status" style="text-align:left; white-space:nowrap;">
						...
						<?php if (in_array($order->status, array('pending', 'failed'))) : ?>
							<a href="<?php echo esc_url( $order->get_cancel_order_url() ); ?>" class="cancel" title="<?php _e('Click to cancel this order', GETTEXT_DOMAIN); ?>">(<?php _e('Cancel', GETTEXT_DOMAIN); ?>)</a>
						<?php endif; ?>
					</td>
					<td class="order-actions" style="text-align:right; white-space:nowrap;">
						...
						<a href="<?php echo esc_url( add_query_arg('order', $order->id, get_permalink(woocommerce_get_page_id('view_order'))) ); ?>" class="button"><?php _e('View', GETTEXT_DOMAIN); ?></a>
...
?>
	<p><?php _e('You have no recent orders.', GETTEXT_DOMAIN); ?></p>
<?php
endif;
?>
