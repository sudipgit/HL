...
<?php else : ?>
	...
	<form action="<?php echo esc_url( add_query_arg( 'address', $load_address, get_permalink( woocommerce_get_page_id('edit_address') ) ) ); ?>" method="post">
		...
		<h4><?php if ($load_address=='billing') _e('Billing Address', GETTEXT_DOMAIN); else _e('Shipping Address', GETTEXT_DOMAIN); ?></h4>
		...
		<input type="submit" class="button" name="save_address" value="<?php _e('Save Address', GETTEXT_DOMAIN); ?>" />
