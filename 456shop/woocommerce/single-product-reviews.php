...
<?php if ( comments_open() ) : ?><div id="reviews"><?php
	...
	if ( $count > 0 ) :
		...
		echo '<div class="star-rating" title="'.sprintf(__('Rated %s out of 5', GETTEXT_DOMAIN), $average).'"><span style="width:'.($average*16).'px"><span itemprop="ratingValue" class="rating">'.$average.'</span> '.__('out of 5', GETTEXT_DOMAIN).'</span></div>';

		echo '<h4>'.sprintf( _n('%s review for %s', '%s reviews for %s', $count, GETTEXT_DOMAIN), '<span itemprop="ratingCount" class="count">'.$count.'</span>', wptexturize($post->post_title) ).'</h4>';

		echo '</div>';

	else :

		echo '<h4>'.__('Reviews', GETTEXT_DOMAIN).'</h4>';
	...
	if ( have_comments() ) :
		...
		echo '<p class="add_review"><a href="#review_form" class="inline show_review_form button">'.__('Add Review', GETTEXT_DOMAIN).'</a></p>';
		...
		echo '<p>'.__('There are no reviews yet, would you like to <a href="#review_form" class="inline show_review_form">submit yours</a>?', GETTEXT_DOMAIN).'</p>';
	...
	$comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment">' . __( 'Your Review', GETTEXT_DOMAIN ) . '</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>' . $woocommerce->nonce_field('comment_rating', true, false);
