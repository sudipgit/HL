<?php
	...
	class UniteImageViewBiz{
		...
		private function outputImage($filepath){
			...
			$info = UniteFunctionsBiz::getPathInfo($filepath);
			$ext = $info["extension"];				
			...
			switch($this->effect){
				...
				case self::EFFECT_BRIGHTNESS:
					if(defined("IMG_FILTER_BRIGHTNESS")){
						...
						UniteFunctionsBiz::validateNumeric($this->effect_arg1,"'ea1' argument");
	...
	private function effect_Blur(&$gdimg, $radius=0.5) {
		...
		public function showImageFromGet(){
			...
			$imageFilename = UniteFunctionsBiz::getGetVar("img");
			$maxWidth = UniteFunctionsBiz::getGetVar("w",-1);
			$maxHeight = UniteFunctionsBiz::getGetVar("h",-1);
			$type = UniteFunctionsBiz::getGetVar("t","");
			
			//set effect
			$effect = UniteFunctionsBiz::getGetVar("e");
			$effectArgument1 = UniteFunctionsBiz::getGetVar("ea1");
