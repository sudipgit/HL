...
jQuery(document).ready(function() {
	...
	jQuery("#pp_course_<?php echo $id;?>").result(function(event, data, formatted) {
		if(data[2] != ''){
			...
			//alert(data[3]);
			jQuery("#tag_product_id").val(data[3]);
		}
		...
		<form autocomplete="off" action="<?php echo get_permalink(get_option('woocommerce_search_page_id'));?>" method="get" class="fr_search_widget" id="fr_pp_search_widget_<?php echo $id;?>">
			...
			<input type="text" id="pp_course_<?php echo $id;?>" onblur="if (this.value == '') {this.value = '<?php echo $search_box_text; ?>';}" onfocus="if (this.value == '<?php echo $search_box_text; ?>') {this.value = '';}" value="<?php echo $search_box_text; ?>" name="rs" class="txt_livesearch" placeholder="Start typing the product name"/>
            </div>
            <?php			
			if ($cat_slug != '') { ?>
