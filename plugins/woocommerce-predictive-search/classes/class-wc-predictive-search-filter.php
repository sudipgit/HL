		...
		if ($search_keyword != '') {
			...
			if ( $search_products && count($search_products) > 0 ) {
				...
				foreach ( $search_products as $product ) {
					...
					$item = '<div class="ajax_search_content"><div class="result_row"><span class="rs_avatar">'.$avatar.'</span><div class="rs_content_popup"><span class="rs_name">'.stripslashes( $product->post_title).'</span>'.$price_html.'<span class="rs_description">'.$product_description.'</span></div></div></div>';
					echo $item.'[|]'.$link_detail.'[|]'.stripslashes( $product->post_title).'[|]'.$product->ID."\n";
					$end_row--;
					if ($end_row < 1) break;
				}
				$rs_item = '';
