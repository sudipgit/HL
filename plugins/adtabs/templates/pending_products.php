	...
	<?php
          $brandids=getAdAllDropBrands();
			...
			if(isset($_POST['is_search']))
			{
			 ...
			 $productlist=getAdminProducts($current_user->ID,$orderby,'draft');
			 $total_p=0;
			 $count_p=0;
			$products=array(); 
		   if($productlist)
		...
		<form name="sort-form" id="sort_form" action="" method="post"/>
			...
			<div class="separator top form-inline small">
				...
				<div class="pagination pagination-small pull-right" style="margin: 0;">
					<ul>
						<li class="<?php if($current==1) echo 'disabled';?>"><a href="<?php bloginfo('url');?>/wp-admin/admin.php?page=pending_products&pp=<?php echo $prev;?>">&laquo;</a></li>
						<?php for($i=1;$i<=$total_page;$i++){?>
						<li class="<?php if($i==$current) echo 'active';?>"><a href="<?php bloginfo('url');?>/wp-admin/admin.php?page=pending_products&pp=<?php echo $i;?>"><?php echo $i;?></a></li>
						<?php } ?>
						
						<li class="<?php if($current==$total_page) echo 'disabled';?>"><a href="<?php bloginfo('url');?>/wp-admin/admin.php?page=pending_products&pp=<?php echo $next;?>">&raquo;</a></li>
					</ul>
				</div>
				<div class="clearfix"></div>
				<!-- // Pagination END -->
