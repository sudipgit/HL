<?php
/**
 * Retrieves and formats items from Facebook page feed for use in a jNewsticker instance
 * 
 * @author Eric Daams <eric@164a.com>
 */

class jNewsticker_Facebook extends jNewsticker_Rss {
    ...
    public function __construct($jnewsticker = null) {
        ...
        $this->id = 'jnews_source_facebook';
        $this->title = __('Facebook Page', 'jnews');
        $this->name = 'jNewsticker_Facebook';
        $this->assist = __('Displays most items from a Facebook page', 'jnews');
        ...
        $this->variables = array(
            ...
            'profile_link' => array(
                'description' => __('Display link to Facebook page', 'jnews'),
        ...
        $this->set_default_format( sprintf( __( "%s posted on %s", 'jnews' ), '%excerpt%', '<a href="%link%">'.__( 'Facebook', 'jnews' ).'</a> %time_ago%' ) );
    }
    ...
    public function jnewsticker_ticker_settings($settings, $ticker_id) {
        // echo $ticker_id;
        //die;
        if ( isset( $settings['data_sources']['jNewsticker_Facebook']['enable'] ) && $settings['data_sources']['jNewsticker_Facebook']['enable'] == 'on' ) {
            foreach ( $settings['data_sources']['jNewsticker_Facebook']['instance'] as $instance => $instance_settings ) {
                ...
                if ( !isset( $instance_settings['profile'] ) ) {
                    ...
                    // echo $profile === false ? 'false' : 'true';
                    // echo '<br />';
                    // echo $instance_settings['profile_name'];
                ...
                $settings['data_sources']['jNewsticker_Facebook']['instance'][$instance] = $instance_settings;
            }
        }        
    ...
    public function get_items() {
        ...
        foreach ( $settings['instance'] as $instance => $instance_settings ) {
            ...
            $transient_key = 'jnewsticker_'.$this->jnewsticker->get_ticker_id().'_facebook_'.$instance;
            ...
            if ( $items === false ) {                
                ...
                $url = array_key_exists( 'feed_url', $instance_settings ) ? $instance_settings['feed_url'] : self::get_feed_url($instance_settings);
                $rss = fetch_feed( $url );
    ...
    public function get_settings() {
        ...
        $settings[] = array(
            'id' => 'profile_name',
            'title' => __('Facebook username', 'jnews'),
            'type' => 'text'
        );
        ...
        $settings[] = array(
            'id' => 'cache',
            'title' => __('How many minutes should results be cached for?', 'jnews'),
            'type' => 'number',
            'default' => 5
        );
    ...
    public function get_profile_url( SimplePie_Item $item ) {
        $settings = $this->get_instance_settings();       
        return isset( $settings['instance'][$item->jnews_instance]['profile'] ) ? $settings['instance'][$item->jnews_instance]['profile']->link : '';
    }
